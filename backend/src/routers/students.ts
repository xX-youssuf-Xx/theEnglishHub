...
import { db } from '../db';
import { courses, courseLevels, classes, studentEnrollments } from '../db/schema';
...
export const studentRouter = router({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (input.enrollments && input.enrollments.length > 0) {
        ...
        const courseIdMap = new Map(courseRecords.map(c => [c.publicId, c.id]));
        const levelIdMap = new Map(levelRecords.map(l => [l.publicId, l.id]));

        enrollmentsData = input.enrollments.map(e => ({
  ...
  enrollInCourse: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (classData.courseId !== course.id || classData.levelId !== level.id) {
        throw new Error('The selected class does not match the course and level');
      }
  ...
  completeCourse: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      return studentService.completeCourse(input.studentId, input.enrollmentId, {
  ...
  dropCourse: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      return studentService.dropCourse(input.studentId, input.enrollmentId, input.reason);
    }),
