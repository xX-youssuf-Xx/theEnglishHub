...
async function applyMigration() {
  ...
  try {
    ...
    try {
      await client.query(`ALTER TYPE "public"."payment_status" ADD VALUE 'cancelled'`);
    ...
    await client.query(`ALTER TABLE "student_payments" ALTER COLUMN "status" SET DEFAULT 'pending'`);
    console.log('✓ Updated student_payments default status to pending');

    // Add price_per_month to course_levels
    try {
      await client.query(`ALTER TABLE "course_levels" ADD COLUMN "price_per_month" numeric(10, 2) DEFAULT '0' NOT NULL`);
    ...
    try {
      await client.query(`ALTER TABLE "student_payments" ADD COLUMN "auto_generated" boolean DEFAULT false NOT NULL`);
    ...
    try {
      await client.query(`ALTER TABLE "student_payments" ADD COLUMN "cycle_number" integer DEFAULT 1`);
    ...
    try {
      await client.query(`ALTER TABLE "teacher_payments" ADD COLUMN "status" "payment_status" DEFAULT 'pending' NOT NULL`);
    ...
    try {
      await client.query(`ALTER TABLE "teacher_payments" ADD COLUMN "auto_generated" boolean DEFAULT false NOT NULL`);
      console.log('✓ Added auto_generated to teacher_payments');
    } catch (e: any) {
      if (e.message.includes('already exists')) {
        console.log('✓ auto_generated column already exists in teacher_payments');
      } else {
        throw e;
      }
    }
    ...
    try {
      await client.query(`ALTER TABLE "teacher_payments" ADD COLUMN "cycle_number" integer DEFAULT 1`);
