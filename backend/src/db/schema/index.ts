...
export const teacherPaymentCycleEnum = pgEnum("teacher_payment_cycle", [
	"4",
	"8",
]);
...
export const coursePrerequisites = pgTable(
	...
	(table) => ({
		uniqueCoursePrerequisite: unique().on(
			table.courseId,
...
export const levelPrerequisites = pgTable(
	"level_prerequisites",
	{
		...
		levelId: integer("level_id")
			.notNull()
			.references(() => courseLevels.id, { onDelete: "cascade" }),
		prerequisiteLevelId: integer("prerequisite_level_id")
			.notNull()
			.references(() => courseLevels.id, { onDelete: "cascade" }),
		createdAt: timestamp("created_at", { withTimezone: true })
			.notNull()
...
export const classTeacherPayments = pgTable(
	"class_teacher_payments",
	{
		...
		paymentCycle: teacherPaymentCycleEnum("payment_cycle")
			.notNull()
			.default("4"),
		isActive: boolean("is_active").notNull().default(true),
		createdAt: timestamp("created_at", { withTimezone: true })
...
export const enrollmentHistory = pgTable("enrollment_history", {
	...
	previousClassId: integer("previous_class_id").references(() => classes.id, {
		onDelete: "set null",
	}),
	newClassId: integer("new_class_id")
		.notNull()
		.references(() => classes.id, { onDelete: "cascade" }),
	previousLevelId: integer("previous_level_id").references(
		() => courseLevels.id,
		{ onDelete: "set null" },
	),
	newLevelId: integer("new_level_id")
		.notNull()
		.references(() => courseLevels.id, { onDelete: "cascade" }),
...
export const studentSessionAttendance = pgTable(
...
export const usersRelations = relations(users, ({ many }) => ({
	...
	studentPaymentsRecorded: many(studentPayments, {
		relationName: "recordedBy",
	}),
	teacherPaymentsRecorded: many(teacherPayments, {
		relationName: "recordedBy",
	}),
...
export const courseLevelsRelations = relations(
	courseLevels,
	({ one, many }) => ({
		course: one(courses, {
		...
		prerequisiteFor: many(levelPrerequisites, {
...
export const levelPrerequisitesRelations = relations(
	levelPrerequisites,
	({ one }) => ({
		course: one(courses, {
...
export const classTeacherPaymentsRelations = relations(
...
export const enrollmentHistoryRelations = relations(
	enrollmentHistory,
	({ one }) => ({
...
export const studentSessionAttendanceRelations = relations(
