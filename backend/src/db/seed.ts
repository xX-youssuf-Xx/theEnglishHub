...
import { users, teachers, courses, courseLevels, books, classes, classSchedules, expenseCategories } from './schema';
...
async function seed() {
  ...
  await db.insert(expenseCategories).values([
    ...
    { name: 'Transportation', description: 'Transportation and logistics costs' },
  ...
  const [level1] = await db.insert(courseLevels).values({
    courseId: course.id,
    levelNumber: 1,
    durationMonths: 4,
  ...
  const [level2] = await db.insert(courseLevels).values({
    courseId: course.id,
    levelNumber: 2,
    durationMonths: 4,
  ...
  const [level3] = await db.insert(courseLevels).values({
    courseId: course.id,
    levelNumber: 3,
    durationMonths: 4,
