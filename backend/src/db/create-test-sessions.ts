  ...
  try {
    ...
    for (const schedule of classesResult.rows) {
      ...
      if (existingSession.rows.length === 0) {
        ...
        sessionsCreated++;
        console.log(`Created session: ${schedule.name} on ${dateStr} at ${schedule.start_time}`);
      }
    }
    ...
    if (classResult.rows.length > 0) {
      ...
      if (levelsResult.rows.length > 0) {
          ...
          if (existingStudent.rows.length === 0) {
            const studentResult = await client.query(
              `INSERT INTO students (full_name, parent_name, parent_phone, address, emergency_contact, class_id)
               ...
               RETURNING id`,
              [studentData.name, studentData.parent, studentData.phone, testClass.id]
            );
        ...
        console.log(`Created attendance records for ${sessionsResult.rows.length} sessions`);
      }
    }
