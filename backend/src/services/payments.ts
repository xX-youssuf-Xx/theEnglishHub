...
export class PaymentService {
	...
	async getStudentPayments(params: {
		...
		try {
			...
			if (params.courseId) {
				whereClause = whereClause
					? and(whereClause, eq(studentPayments.courseId, params.courseId))
					: eq(studentPayments.courseId, params.courseId);
			}
				...
				pendingPayments.map(async (p) => {
					...
					return {
						...
						class: p.class
							...
							: null,
						course: p.class?.course
							? { id: p.class.course.publicId, name: p.class.course.name }
							: null,
						level: p.class?.level
							? {
									id: p.class.level.publicId,
									levelNumber: p.class.level.levelNumber,
								}
							: null,
						amount: p.amount,
						sessionsCovered: p.sessionsCovered,
			...
			for (const payment of pendingPayments) {
				...
				group.payments.push({
					...
					class: payment.class
						...
						: null,
					course: payment.class?.course
						? {
								id: payment.class.course.publicId,
								name: payment.class.course.name,
							}
						: null,
					level: payment.class?.level
						? {
								id: payment.class.level.publicId,
								levelNumber: payment.class.level.levelNumber,
							}
						: null,
