import { db } from '../db';
import { studentPayments, teacherPayments, expenses, students, classes, courses, teachers, sessions, classTeacherPayments } from '../db/schema';
import { eq, and, desc, gte, lte, sql, sum, count, or } from 'drizzle-orm';
...
export class PaymentService {
  ...
  async getPaymentHistory(params: {
    ...
    try {
      ...
      if (type === 'all' || type === 'student') {
        ...
        allPayments.push(...studentData.map(p => ({
          id: p.publicId,
          ...
          amount: p.amount,
          status: p.status,
          person: p.student ? { id: p.student.publicId, name: p.student.fullName } : null,
          class: p.class ? { id: p.class.publicId, name: p.class.name } : null,
          course: p.course ? { id: p.course.publicId, name: p.course.name } : null,
          sessionsCovered: p.sessionsCovered,
          invoiceNumber: p.invoiceNumber,
        ...
        const teacherPaymentsWithCycle = await Promise.all(
          teacherData.map(async (p) => {
            ...
            return {
              ...
              amount: p.amount,
              status: p.status,
              person: p.teacher ? { id: p.teacher.publicId, name: p.teacher.fullName } : null,
              class: p.class ? { id: p.class.publicId, name: p.class.name } : null,
              course: null,
              sessionsCovered: p.sessionsCovered,
              invoiceNumber: null,
      ...
      if (type === 'all' || type === 'expense') {
        ...
        allPayments.push(...expenseData.map(e => ({
          id: e.publicId,
    ...
    try {
      ...
      for (const payment of pendingPayments) {
        ...
        group.payments.push({
          id: payment.publicId,
          amount: payment.amount,
          class: payment.class ? { id: payment.class.publicId, name: payment.class.name } : null,
  ...
  async getPendingPaymentsByCourse() {
    try {
      ...
      for (const payment of pendingPayments) {
        ...
        group.payments.push({
          id: payment.publicId,
          student: payment.student ? { id: payment.student.publicId, fullName: payment.student.fullName } : null,
          class: payment.class ? { id: payment.class.publicId, name: payment.class.name } : null,
  ...
  async bulkSettleTeacherPayments(data: {
    ...
    try {
      ...
      logger.info(`Bulk settled ${settledPaymentIds.length} teacher payments for teacher ${data.teacherId}`, {
        teacherId: data.teacherId,
