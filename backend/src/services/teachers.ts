import { db } from '../db';
import { teachers, classes, classTeacherPayments, teacherPayments, sessions, courses, courseLevels } from '../db/schema';
...
export class TeacherService {
  async getAll(params: {
    ...
    try {
      ...
      if (params.search) {
        whereClause = and(
          whereClause,
          like(teachers.fullName, `%${params.search}%`)
        ) as any;
      }
  ...
  async getById(publicId: string) {
    try {
      ...
      const upcomingPayments = teacher.classPayments?.map(cp => {
        const completedSessions = cp.class.sessions?.filter(s => s.status === 'completed').length || 0;
        const cycleSize = parseInt(cp.paymentCycle);
        ...
        const paidAmount = teacher.payments
  ...
  async assignToClass(teacherPublicId: string, data: {
    classId: number;
    paymentAmount: number;
    paymentCycle: '4' | '8';
  }) {
    ...
    try {
      ...
      classAssignments.forEach((assignment) => {
        ...
        coursesMap.get(courseId).classes.push({
          ...
          schedules: assignment.class.schedules?.filter(s => s.isActive).map(s => ({
