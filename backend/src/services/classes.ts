import { db } from '../db';
import { classes, sessions, students, classTeacherPayments, teachers } from '../db/schema';
...
export class ClassService {
  async getAll(params: {
    ...
    try {
      ...
      if (params.teacherId) {
        whereClause = and(whereClause, eq(classes.teacherId, params.teacherId)) as any;
      }
  ...
  async getById(publicId: string) {
    try {
      ...
      const completedSessions = classData.sessions?.filter(s => s.completed).length || 0;
  ...
  async create(data: {
    ...
    try {
      const [classData] = await db.insert(classes)
        .values({
          name: data.name,
  ...
  async createSession(classPublicId: string, data: {
    sessionDate: Date;
    startTime?: string;
    endTime?: string;
    notes?: string;
