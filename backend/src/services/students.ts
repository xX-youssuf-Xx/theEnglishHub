	...
	async getAll(params: {
		...
		try {
			...
			if (params.search) {
				...
				const parentNameMatch = buildLooseFuzzyCondition(
					students.parentName,
					params.search,
				);
				const parentPhoneMatch = buildLooseFuzzyCondition(
				...
				whereClause = and(
					whereClause,
					or(nameMatch, parentNameMatch, parentPhoneMatch),
				) as any;
			}
			...
			const calculatePaymentStatus = (
				...
				const remainingSessions =
					latestPayment.sessionsCovered - sessionsConsumed;
			...
			return {
				data: data.map((s) => {
					const activeEnrollment =
						s.enrollments?.find((e) => e.status === "active") || null;
	...
	async enrollInCourse(
		...
		try {
			...
			if (existingActiveEnrollment) {
				throw new Error(
					"Student is already enrolled in a course. Please complete or drop the current course before enrolling in a new one.",
				);
			}
			...
			if (
				...
				classData.levelId !== data.levelId
			) {
			...
			const enrollment = await db.transaction(async (tx) => {
				...
				if (levelBooks && levelBooks.length > 0) {
					...
					if (totalBookPrice > 0) {
						...
						logger.info(
							`Generated book fee payment of ${totalBookPrice} for student ${student.id}, level ${data.levelId}`,
						);
					}
				}
			...
			try {
				...
				logger.error(
					"Failed to insert enrollment history record:",
					historyError,
				);
			}
	...
	async getCourseHistory(studentPublicId: string) {
		try {
			...
			for (const enrollment of enrollments) {
				...
				const relatedLevels =
					levelIds.length > 0
						? await db.query.courseLevels.findMany({
								where: inArray(courseLevels.id, levelIds),
							})
						: [];
				...
				const enrichedHistory = historyRows.map((h) => ({
					...h,
					previousClass: h.previousClassId
						? classMap.get(h.previousClassId) || null
						: null,
					newClass: h.newClassId ? classMap.get(h.newClassId) || null : null,
					previousLevel: h.previousLevelId
						? levelMap.get(h.previousLevelId) || null
						: null,
			...
			return {
				...
				history: enrollments.map((e) => ({
					...
					notes: e.notes,
					changes:
						(historyByEnrollment.get(e.id) || []).map((h) => ({
							id: h.publicId,
		...
		try {
			...
			if (levelBooks && levelBooks.length > 0) {
				...
				if (totalBookPrice > 0) {
					...
					logger.info(
						`Generated book fee payment of ${totalBookPrice} for student ${student.id}, new level ${newLevelId}`,
					);
				}
			}
