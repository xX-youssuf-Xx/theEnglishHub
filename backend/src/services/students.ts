import { db } from '../db';
import { students, studentEnrollments, studentPayments, classes, courses, courseLevels, sessions } from '../db/schema';
...
export class StudentService {
  ...
  async changeClass(studentPublicId: string, newClassId: number) {
    try {
      const student = await db.query.students.findFirst({
        where: eq(students.publicId, studentPublicId),
      });
  ...
  async advanceLevel(studentPublicId: string, enrollmentPublicId: string, newLevelId: number) {
    try {
      const student = await db.query.students.findFirst({
        where: eq(students.publicId, studentPublicId),
      });
      ...
      await db.update(studentEnrollments)
        .set({ currentLevelId: newLevelId, updatedAt: new Date() })
        .where(eq(studentEnrollments.id, enrollment.id));
