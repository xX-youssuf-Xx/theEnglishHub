  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-text-heading">إدارة الكورسات</h1>
          <p className="text-text-muted mt-1">إدارة الكورسات والمستويات والكلاسات</p>
        </div>
                        ...
                        <TableRow key={course.id} className="h-14">
                          <TableCell className="py-2">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="font-medium text-text-heading">{course.name}</p>
                                {course.description && (
                          ...
                          <TableCell className="py-2">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm">{course.levels?.length || 0} مستوى</span>
                            </div>
                          </TableCell>
                          <TableCell className="py-2">
                            <div className="flex items-center gap-2">
                              <Users className="w-4 h-4 text-text-muted" />
                              <span className="text-sm">{course.enrollmentCount || 0} طالب</span>
                            </div>
                          </TableCell>
                          <TableCell className="py-2">
                            <div className="flex items-center gap-2">
                              <Clock className="w-4 h-4 text-text-muted" />
                              <span className="text-sm">{course.sessionsPerMonth || 4} حصة</span>
                            </div>
                          </TableCell>
                          <TableCell className="py-2">
                          ...
                          <TableCell className="text-left py-2">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                <DropdownMenuItem onClick={() => handleView(course.id, course.name)}>
                                  <Eye className="w-4 h-4 ml-2" />
                                  عرض التفاصيل
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => handleViewClasses(course.id, course.name)}>
                                  <GraduationCap className="w-4 h-4 ml-2" />
                                  عرض الكلاسات
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => handleAddLevel(course.id, course.name)}>
                                  <Layers className="w-4 h-4 ml-2" />
                                  إضافة مستوى
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => handleEdit(course.id, course.name)}>
