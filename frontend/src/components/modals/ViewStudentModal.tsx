...
import { Loader2, User, Phone, MapPin, AlertCircle, BookOpen, Users, Calendar } from 'lucide-react';
...
export function ViewStudentModal({ studentId, open, onOpenChange }: ViewStudentModalProps) {
  const { data: student, isLoading, error } = trpc.students.getById.useQuery(
    studentId || '',
    {
            ...
            <Card>
              ...
              <CardContent className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{student.age ? `${student.age} سنة` : 'غير محدد'}</p>
                </div>
                <div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <p className="font-medium">{student.class?.name || 'غير مسجل'}</p>
                </div>
                
                {student.enrollments && student.enrollments.length > 0 && (
                  <div>
                    <p className="text-sm text-text-muted mb-2">الكورسات المسجل فيها:</p>
                    <div className="space-y-2">
                      {student.enrollments.map((enrollment) => (
                        <div key={enrollment.id} className="p-3 bg-background-page rounded-lg">
                          <div className="flex justify-between items-center">
                            <span className="font-medium">{enrollment.course?.name}</span>
                            <Badge variant="outline">{enrollment.currentLevel?.levelNumber}</Badge>
                          </div>
                        </div>
                      ))}
                    </div>
