...
export function RecordPaymentModal({
	...
	const { data: student } = trpc.students.getById.useQuery(studentId || "", {
		enabled: !!studentId && open,
	});

	// Get sessionsPerMonth from student's course
	const sessionsPerMonth =
		student?.enrollments?.[0]?.course?.sessionsPerMonth || 4;
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (
			...
			setError(
				"جميع الحقول المطلوبة يجب ملؤها والطالب يجب أن يكون مسجل في كلاس وكورس",
			);
			return;
		}
		...
		recordPayment.mutate({
			...
			sessionsCovered:
				formData.type === "tuition" ? sessionsPerMonth : undefined,
					...
					{formData.type === "tuition" && student?.enrollments?.[0]?.course && (
						<div className="p-3 bg-muted rounded-lg">
							<p className="text-sm text-muted-foreground">
								سيتم تسجيل الدفعة لتغطية <strong>{sessionsPerMonth} حصص</strong>{" "}
								(حسب إعدادات الكورس)
							</p>
						</div>
					)}
