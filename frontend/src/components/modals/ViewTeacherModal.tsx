...
import { GraduationCap, Phone, Mail, MapPin, Loader2, Users, Wallet } from 'lucide-react';
...
export function ViewTeacherModal({ isOpen, onClose, teacherId }: ViewTeacherModalProps) {
        ...
        {isLoading ? (
          ...
          <div className="space-y-6 mt-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-xl font-bold text-text-heading">{teacher.fullName}</h3>
              ...
              {teacher.classes && teacher.classes.length > 0 ? (
                <div className="space-y-2">
                  {teacher.classes.map((cls: any) => (
                    <div key={cls.id} className="p-3 bg-bg-light rounded-lg flex justify-between items-center">
                      <span className="font-medium">{cls.name}</span>
                      <div className="flex items-center gap-2 text-sm text-text-muted">
                        <Wallet className="w-4 h-4" />
                        <span>{cls.paymentAmount} جنيه</span>
                        <span className="text-xs">(كل {cls.paymentCycle} حصص)</span>
                      </div>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-text-muted text-center py-4">لا يوجد كلاسات مسندة</p>
              )}
            </div>
