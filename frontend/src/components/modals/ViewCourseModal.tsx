...
import { Loader2, BookOpen, Layers, Users, Clock, Link2 } from 'lucide-react';
...
export function ViewCourseModal({ courseId, open, onOpenChange }: ViewCourseModalProps) {
        ...
        {isLoading ? (
          ...
          <div className="space-y-6 mt-4">
            ...
            <div className="bg-primary/5 p-4 rounded-lg">
              <h3 className="text-xl font-bold text-text-heading">{course.name}</h3>
              {course.description && (
            ...
            {course.syllabus && (
              <Card>
                ...
                <CardContent>
                  <p className="text-sm text-text-muted whitespace-pre-line">{course.syllabus}</p>
                </CardContent>
              </Card>
            )}
            ...
            <div className="space-y-3">
              <h3 className="font-semibold">المستويات ({course.levels?.length || 0})</h3>
                      ...
                      <Card>
                          ...
                          <CardHeader className="pb-2 cursor-pointer hover:bg-muted/50 transition-colors">
                            <CardTitle className="text-base flex items-center justify-between">
                               <div className="flex items-center gap-2">
                                ...
                                {level.pricePerMonth && level.pricePerMonth !== '0' && (
                        ...
                        <CollapsibleContent>
                          <CardContent className="pt-0">
                            {level.description && (
                              <p className="text-sm text-text-muted mb-3">{level.description}</p>
                            )}
                            
                            {level.prerequisites?.length > 0 && (
                              <div className="mb-3">
                                <p className="text-xs text-text-muted mb-1">المتطلبات:</p>
                            ...
                            {level.books?.length > 0 && (
                              <div>
                                <p className="text-xs text-text-muted mb-1">الكتب:</p>
                                <ul className="text-sm space-y-1">
                                  {level.books.map((book: any) => (
                                    <li key={book.id} className="flex justify-between">
                                      <span>{book.name}</span>
                                      <span className="text-text-muted">{book.price} جنيه</span>
                                    </li>
                                  ))}
                                </ul>
                              </div>
