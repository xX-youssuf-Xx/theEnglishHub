...
import { BookOpen, Loader2, Link2 } from 'lucide-react';
...
export function AddCourseModal({ isOpen, onClose }: AddCourseModalProps) {
  ...
  const existingCourses = coursesData?.data?.filter((c: any) => c.isActive) || [];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate({
      ...
      prerequisiteCourseIds: selectedPrerequisites.length > 0 ? selectedPrerequisites : undefined,
    });
  };
