...
export function EnrollInCourseModal({ isOpen, onClose, studentId, studentName }: EnrollInCourseModalProps) {
  ...
  const formatSchedule = (schedules: any[]) => {
    ...
    return schedules.map(s => `${daysOfWeek[s.dayOfWeek]} ${s.startTime}-${s.endTime}`).join('، ');
  };
      ...
      <DialogContent className="sm:max-w-[550px]" dir="rtl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          ...
          <div className="space-y-2">
            ...
            >
              <SelectTrigger>
                <SelectValue placeholder={selectedCourseId ? "اختر المستوى" : "اختر الكورس أولاً"} />
              </SelectTrigger>
              <SelectContent>
                {levels.map((level) => (
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button 
              type="submit" 
              disabled={enrollMutation.isPending || !selectedCourseId || !selectedLevelId || !selectedClassId}
            >
